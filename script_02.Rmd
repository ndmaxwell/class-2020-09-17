...
#calculate enrollment for GOV 50
en_fall_2020 %>% 
  filter(course_name == "Data") %>% 
  summarise(sum = sum(u_grad))
...
en_fall_2020 %>% 
  slice(1:10) %>% 
  ggplot(aes(x = reorder(course_title, u_grad), y = u_grad)) +
     geom_col(fill = "steelblue") +
     ggtitle("Undergrad Enrollment of 10 Largest Harvard Courses") +
     labs(x = "Course Title", y = "Undergrad Enrollment") +
     coord_flip()
